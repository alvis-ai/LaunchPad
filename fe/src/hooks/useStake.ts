...
export const useStake = () => {
  ...
  async function approve(contractAddress, amount, decimals = 18) {
    if (!depositTokenContract || !walletAddress || !contractAddress) {
      return Promise.reject();
    }

    const requestedAmount = BigNumber.isBigNumber(amount)
      ? amount
      : parseUnits(String(amount || 0), decimals);
    const fallbackAmount = parseUnits(String(APPROVE_STAKING_AMOUNT_ETHER), decimals);
    const approveAmount = requestedAmount.gt(fallbackAmount) ? requestedAmount : fallbackAmount;

    const currentAllowance = await depositTokenContract.allowance(walletAddress, contractAddress);
    ...
    return (
      (depositTokenContract.approve &&
        depositTokenContract.approve(contractAddress, approveAmount))
        .then(transaction => {
