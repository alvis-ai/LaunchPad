...
export default function LivePoolCard(props: LivePoolCardProds) {
  ...
  const tokenPriceInUsd: number = useMemo(() => {
    const paymentTokenDecimals = props?.info?.paymentTokenDecimals || 18;
    const tokenPriceInETH = props?.info?.tokenPriceInPT ? Number(props.info.tokenPriceInPT) / Math.pow(10, paymentTokenDecimals) : 0;
