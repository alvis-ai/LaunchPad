...
async function isJsException(promise, errorMsg) {
  try {
    let x = await promise;
  } catch (e) {
    msg = e.message;
    return msg.includes(errorMsg);
  }
}
describe("SalesFactory", function () {
  let Admin;
  let LaunchPadSale;
  let LaunchPadToken, BreToken2;
  let SalesFactory;
  let LaunchPadSaleFactory;
  ...
  beforeEach(async function () {
    ...
    BreToken2 = await BreTokenFactory.deploy(
      "Bre2",
      "BRE2",
      ethers.parseUnits("100000000"),
      18
    ...
    LaunchPadSaleFactory = await ethers.getContractFactory("LaunchPadSale");
  });
  ...
  context("Setup", async function () {
    ...
    describe("Set allocation staking", async function () {
      it("Should set allocation staking contract", async function () {
        // When
        await SalesFactory.setAllocationStaking(await LaunchPadToken.getAddress());

        // Then
        expect(await SalesFactory.allocationStaking()).to.equal(
          await LaunchPadToken.getAddress()
        );
      });
      ...
      it("Should not set allocation staking contract to zero address", async function () {
        // Then
        await expect(
          SalesFactory.setAllocationStaking(ZERO_ADDRESS)
        ).to.be.reverted;
      });
      ...
      it("Should not allow non-admin to set allocation staking contract", async function () {
        ...
        // Then
        await expect(
          SalesFactory.setAllocationStaking(await LaunchPadToken.getAddress())
        ).to.be.reverted;
      });
    });
  });
  ...
  context("Sales", async function () {
    describe("Deploy sale", async function () {
      it("Should deploy sale", async function () {
        ...
        expect(
          await SalesFactory.isSaleCreatedThroughFactory(saleAddress)
        ).to.be.true;
      });
      ...
      it("Should not allow non-admin to deploy sale", async function () {
        ...
        // Then
        await expect(SalesFactory.deploySale()).to.be.revertedWith(
      ...
      it("Should emit SaleDeployed event", async function () {
        // Then
        await expect(SalesFactory.deploySale()).to.emit(
          SalesFactory,
          "SaleDeployed"
        );
      });
    ...
    describe("Set sale owner and token", async function () {
      it("Should set sale owner and token", async function () {
        // Given
        await SalesFactory.deploySale();
        const LaunchPadSale = LaunchPadSaleFactory.attach(await SalesFactory.allSales(0));
      ...
      expect(sale.token).to.equal(await LaunchPadToken.getAddress());
      expect(await SalesFactory.saleOwnerToSale(deployer.address)).to.equal(
        await LaunchPadSale.getAddress()
      );
      expect(await SalesFactory.tokenToSale(await LaunchPadToken.getAddress())).to.equal(
        await LaunchPadSale.getAddress()
      );
      });
      ...
      it("Should not allow same sale owner to own two sales", async function () {
        ...
        // When
        await SalesFactory.deploySale();
        const LaunchPadSale2 = LaunchPadSaleFactory.attach(await SalesFactory.allSales(1));

        // Then
        await expect(
          LaunchPadSale2.setSaleParams(
            await BreToken2.getAddress(),
            deployer.address,
            10,
            10,
            blockTimestamp + 100,
      ...
      it("Should not allow address to set sale owner and token if address not deployed through factory", async function () {
        ...
        // Then
        await expect(
          SalesFactory.setSaleOwnerAndToken(deployer.address, await LaunchPadToken.getAddress())
    ...
    describe("Get all sales", async function () {
      ...
      it("Should not allow negative start index", async function () {
        ...
        // Then
        //在调用abi执行时就报错(参数必须时uint)，所以用try-catch
        expect(
          await isJsException(SalesFactory.getAllSales(-5, 2), "out-of-bounds")
        ).to.true;
      });
